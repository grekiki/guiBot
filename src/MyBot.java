...
/**
 * Initial implementation keeps picking random locations on the map and sending units there. Worker units collect resources if they see them while warrior units shoot if they see opponents.
 */
public class MyBot implements Bot{
	...
	@Override public void update(GameState state,Api api){
		//we let the drawing start and give it data
		rendering.state=state;
		if(!rendering.gameStarted){
			rendering.gameStarted=true;
		}
		try{
			Thread.sleep(10);
